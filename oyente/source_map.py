...
class SourceMap:
    ...
    def _get_sig_to_func_by_contract(cls):
        if cls.allow_paths:
            cmd = 'lityc --disable-oyente --combined-json hashes %s %s --allow-paths %s' % (cls.remap, cls.parent_filename, cls.allow_paths)
        else:
            cmd = 'lityc --disable-oyente --combined-json hashes %s %s' % (cls.remap, cls.parent_filename)
        out = run_command(cmd)
        out = json.loads(out)
    ...
    def _load_position_groups(cls):
        if cls.allow_paths:
            cmd = "lityc --disable-oyente --combined-json asm %s %s --allow-paths %s" % (cls.remap, cls.parent_filename, cls.allow_paths)
        else:
            cmd = "lityc --disable-oyente --combined-json asm %s %s" % (cls.remap, cls.parent_filename)
        out = run_command(cmd)
        out = json.loads(out)
        return out['contracts']
